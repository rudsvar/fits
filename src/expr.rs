...
pub fn eval(expr: Expr, env: &Env<Value>) -> Result<Value, RuntimeError> {
    Ok(match expr {
        ...
        Expr::Eq(e1, e2) => Value::Bool(eval(*e1, env)? == eval(*e2, env)?),
        Expr::And(e1, e2) => Value::Bool(eval_as::<bool>(*e1, env)? && eval_as::<bool>(*e2, env)?),
        ...
        Expr::Mod(e1, e2) => Value::Int(eval_as::<i128>(*e1, env)? % eval_as::<i128>(*e2, env)?),
